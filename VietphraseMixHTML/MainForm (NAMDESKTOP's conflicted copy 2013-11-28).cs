    ...
    public partial class MainForm : Form
    {
        ...
        private void FictionTranslate(object sender)
        {
            ...
            while (_currentProcessingCount < _maxProcessingCount)
            {
                ...
                while (_processingQueue.Count > 0)
                {
                    ...
                    {
                        //string translated = _contentBuilder.ToString();
                        //if (!dontNeedTranslate) translated = Translate(translated,sender);
                        //SaveToDisk(translated, _stepCount++);
        ...
        private string Translate(string originalContent,object sender)
        {
            ...
            GlobalCache.ThanhNgu.AsEnumerable().ToList().ForEach(t =>
                translateContent = VBStrings.Replace(translateContent, t.Key, t.Value + " "));
            //GlobalCache.ThanhNgu.AsEnumerable().ToList().ForEach(t => translateContent = translateContent.Replace(t.Key, t.Value + " "));
        ...
        private FictionObject CreateNewFictionObject()
        {
            ...
            if (fictionObject.Location.IndexOf("\\") >= 0)
            {
                fictionObject.Location = fictionObject.Location.Substring(fictionObject.Location.LastIndexOf("\\") + 1);
            }
        ...
        private void SaveFormToFictionObject()
        {
            ...
            _currentFictionObject.Location=txtLocation.Text.Substring(txtLocation.Text.LastIndexOf("\\")+1);                        
        ...
        private void BtnSaveFictionObjectClick(object sender, EventArgs e)
        {
            ...
            //{
            //    if (fictionObject.Name.Equals(_currentFictionObject.Name)) break;
            //    if (index < (_fictionObjectManager.ProjectList.Count - 1)) index += 1;
            //}
